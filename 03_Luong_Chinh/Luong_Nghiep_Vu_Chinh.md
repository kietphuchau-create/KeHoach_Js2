...
* **Kịch bản:** Bệnh nhân đã đặt cọc giữ chỗ qua VNPay (1 dòng `payments` với `method = VNPAY`, `status = SUCCEEDED`), nhưng sau đó bác sĩ báo nghỉ đột xuất (xem mục 7.1) hoặc bệnh nhân hủy lịch hợp lệ trước hạn (> 2 giờ theo `CANCELLATION_LIMIT_HOURS`).
* **Xử lý:** `appointment_status_logs` ghi nhận lý do hủy (`from_status=CONFIRMED, to_status=CANCELLED, reason='Bác sĩ nghỉ đột xuất'`). Job hoàn tiền gọi API Refund của VNPay Sandbox, rồi cập nhật đúng **dòng `payments`** đó: `refunded_amount`, `refunded_at`, `refund_ref` và `status = REFUNDED` — chỉ sau khi cổng thanh toán xác nhận thành công.
  * **Chống hoàn tiền 2 lần:** cột `payments.transaction_ref` có ràng buộc **UNIQUE** ở tầng DB, nên webhook của cổng thanh toán gọi lặp cùng một mã giao dịch sẽ bị chặn ngay (`Duplicate entry ... for key 'uq_payment_txn'`) thay vì tạo thêm bản ghi. Ràng buộc `CHECK (refunded_amount <= amount)` chặn luôn trường hợp hoàn quá số tiền đã thu.
  * **Ca khám có nhiều lần thu:** nếu bệnh nhân đã cọc online **và** trả thêm tại quầy thì mỗi lần là 1 dòng `payments` riêng, hoàn tiền xử lý từng dòng độc lập — thiết kế cũ (1 cột `payment_status` duy nhất trên `appointments`) không thể diễn tả tình huống này.
  * Nếu bệnh nhân tự hủy **trong vòng 2 giờ trước giờ hẹn** (vi phạm `CANCELLATION_LIMIT_HOURS`), tiền cọc **không hoàn**: giữ nguyên dòng `payments` ở `SUCCEEDED`, ghi rõ lý do trong `appointment_status_logs.reason` và `payments.note` để tránh khiếu nại.
