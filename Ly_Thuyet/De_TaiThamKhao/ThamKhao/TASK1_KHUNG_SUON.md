...
# 2. Chạy ứng dụng (profile mặc định đã là mysql-xampp, không cần tham số)
cd 05_Y_Tuong_Database_Tai
./gradlew bootRun

# 3. Kiểm thử toàn bộ Task 1 (mở terminal thứ 2)
bash test-task1.sh
```

Flyway tự tạo 17 bảng + dữ liệu mẫu vào database `medsched_db` ngay lần chạy đầu.
**Không cần** import tay file SQL, **không cần** Docker, **không cần** PostgreSQL.
